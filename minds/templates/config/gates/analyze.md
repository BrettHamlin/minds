...
**Rules**:
- If `ANALYSIS_MD` is missing or contains no findings table: respond `ESCALATION` with instruction to re-run `/collab.analyze` so a proper report exists.
...
"Apply the following to the specified files before re-running `.collab/scripts/verify-and-complete.sh analyze 'Analysis phase finished'`:
