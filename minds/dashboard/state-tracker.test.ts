...
describe("MindsStateTracker", () => {
  ...
  test("subscribeRaw: raw callback fires for each event with the original MindsBusMessage", () => {
    ...
    const msg2 = makeMsg(MindsEventType.DRONE_SPAWNED, { waveId: "1", mindName: "transport" });
  ...
  test("subscribeRaw: multiple raw subscribers work independently", () => {
    ...
    const unsubA = tracker.subscribeRaw((event) => {
  ...
  test("subscribeRaw: raw subscription does not interfere with state subscription (both fire)", () => {
    ...
    tracker.subscribe((state) => {
      stateSnapshots.push(state.ticketId);
    });
    ...
    expect(stateSnapshots[0]).toBe("TEST-1");
  });

  test("subscribeRaw: raw callback receives event before state is mutated", () => {
    const tracker = new MindsStateTracker();
    let waveCountDuringRaw = -1;

    tracker.subscribeRaw((_event) => {
      // At the time the raw callback fires, the wave has NOT been added yet
      const state = tracker.getState("TEST-1");
      waveCountDuringRaw = state ? state.waves.length : 0;
    });

    tracker.applyEvent(makeMsg(MindsEventType.WAVE_STARTED, { waveId: "1" }));

    // Raw callback saw 0 waves (state not yet mutated); after applyEvent the wave exists
    expect(waveCountDuringRaw).toBe(0);
