...
import { postQuestionToChannel } from '../plugins/slack/interactions.js';
...
router.post('/channel', asyncHandler(async (req: Request, res: Response) => {
  ...
  // Create Slack channel (handles collision)
  const slackChannel = await createSlackChannel(channelName);

  // Add role members to database (fetch display names from Slack)
  ...
  for (const roleAssignment of roles) {
    ...
    if (role && roleAssignment.members?.length > 0) {
      // Fetch display names for each user from Slack
      const memberInputs = await Promise.all(
        roleAssignment.members.map(async (userId: string) => {
          try {
            ...
            return {
              slackUserId: userId,
              displayName: userId, // Fallback to user ID if fetch fails
            };
          }
        })
      );
  ...
  // Invite all members to channel
  if (memberUserIds.length > 0) {
    await inviteMembers(slackChannel.id, memberUserIds);
  }
  ...
  await createChannelRecord(
    ...
    slackChannel.id,
    slackChannel.name,
    [], // nameSuggestions can be stored from previous step if needed
    slackChannel.name !== channelName
  );

  // Post welcome message
  await postWelcomeMessage(slackChannel.id, spec.title, spec.pmDisplayName || 'PM');
  ...
  // Start Blind QA: Generate and post first question
