...
describe("MindsStateTracker", () => {
  ...
  describe("subscribeRaw()", () => {
    ...
    test("unsubscribe stops delivery of subsequent events", () => {
      const tracker = new MindsStateTracker();
      const received: MindsBusMessage[] = [];
    ...
    test("raw callback receives event before state is mutated", () => {
      const tracker = new MindsStateTracker();
      let wavesAtCallbackTime: number | null = null;

      tracker.subscribeRaw((_event) => {
        // At callback time, the WAVE_STARTED event has NOT yet been applied to state
        const state = tracker.getState("TEST-1");
        wavesAtCallbackTime = state ? state.waves.length : 0;
      });

      tracker.applyEvent(makeMsg(MindsEventType.WAVE_STARTED, { waveId: "1" }));

      // After applyEvent completes, the state has 1 wave
      const finalState = tracker.getState("TEST-1")!;
      expect(finalState.waves).toHaveLength(1);

      // But inside the raw callback, no state existed yet for this ticket
      expect(wavesAtCallbackTime).toBe(0);
    });
  });
});
