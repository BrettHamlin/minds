...
describe("MindsStateTracker", () => {
  ...
  test("subscribeRaw callback fires for each event with the original MindsBusMessage", () => {
    const tracker = new MindsStateTracker();
  ...
  test("multiple raw subscribers work independently", () => {
    ...
    const unsub1 = tracker.subscribeRaw((event) => received1.push(event));
    const unsub2 = tracker.subscribeRaw((event) => received2.push(event));

    const msg = makeMsg(MindsEventType.WAVE_STARTED, { waveId: "1" });
    tracker.applyEvent(msg);
    ...
    // Only subscriber 2 still receives after unsub1
    expect(received1).toHaveLength(1);
  ...
  test("raw callback receives event before state is mutated", () => {
    const tracker = new MindsStateTracker();
    let stateAtCallTime: ReturnType<typeof tracker.getState> = undefined;

    tracker.subscribeRaw((_event) => {
      // Check state for TEST-1 before WAVE_STARTED is processed
      stateAtCallTime = tracker.getState("TEST-1");
    });

    // Before the event, state doesn't exist
    expect(tracker.getState("TEST-1")).toBeUndefined();
    tracker.applyEvent(makeMsg(MindsEventType.WAVE_STARTED, { waveId: "1" }));

    // The raw callback saw the state BEFORE mutation (state didn't exist yet)
    expect(stateAtCallTime).toBeUndefined();
    // But after applyEvent, state now exists
