...
import { questionResponse } from "../coordination/question-response"; // CROSS-MIND
...
describe("collab.install.ts structural checks", () => {
  const installSrc = fs.readFileSync(
    path.join(REAL_REPO_ROOT, "minds/installer/collab-install.ts"),
    "utf-8"
  );
