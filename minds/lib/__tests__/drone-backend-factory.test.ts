 ...
 * 6. Default (no env var) returns TmuxDroneBackend
 */
...
describe("createDroneBackend", () => {
  ...
  it("defaults to TmuxDroneBackend when no env var or forceBackend set", async () => {
    // No env var set, no forceBackend — should default to tmux (no Axon probing)
    const backend = await createDroneBackend({ repoRoot: "/nonexistent/repo" });
    expect(backend).toBeInstanceOf(TmuxDroneBackend);
    backend.close();
  });
