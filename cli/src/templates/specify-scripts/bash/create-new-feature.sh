...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --source-repo)
            if [ $((i + 1)) -gt $# ]; then
                echo 'Error: --source-repo requires a value' >&2
                exit 1
            fi
            i=$((i + 1))
            next_arg="${!i}"
            ...
            SOURCE_REPO="$next_arg"
            ;;
        --help|-h)
            echo "Usage: $0 [--json] [--short-name <name>] [--number N] [--worktree] [--worktree-path <dir>] [--source-repo <path>] <feature_description>"
            echo ""
            echo "Options:"
            echo "  --json                Output in JSON format"
            echo "  --short-name <name>   Provide a custom short name (2-4 words) for the branch"
            echo "  --number N            Specify branch number manually (overrides auto-detection)"
            echo "  --worktree            Create a git worktree instead of switching branches"
            echo "  --worktree-path <dir> Directory for worktrees (default: ../worktrees/ relative to repo root)"
            echo "  --source-repo <path>  Create branch and worktree from this repo instead of the current one"
            echo "  --help, -h            Show this help message"
            echo ""
            echo "Examples:"
            echo "  $0 'Add user authentication system' --short-name 'user-auth'"
            echo "  $0 'Implement OAuth2 integration for API' --number 5"
            echo "  $0 --worktree 'Add feed caching system'"
            echo "  $0 --worktree --worktree-path ~/worktrees 'Add feed caching'"
            exit 0
            ;;
        *)
            ARGS+=("$arg")
...
if [ "$HAS_GIT" = true ]; then
    # Check if we're already on this feature branch (e.g., inside an existing worktree)
    CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "")
    if [ "$CURRENT_BRANCH" = "$BRANCH_NAME" ]; then
        >&2 echo "[specify] Already on branch $BRANCH_NAME, skipping branch creation"
    elif $USE_WORKTREE; then
        # Resolve worktree base directory
        if [ -z "$WORKTREE_PATH" ]; then
            WORKTREE_PATH="$(dirname "$REPO_ROOT")/worktrees"
        fi
        mkdir -p "$WORKTREE_PATH"
        WORKTREE_DIR="$WORKTREE_PATH/$BRANCH_NAME"

        if [ -d "$WORKTREE_DIR" ]; then
            >&2 echo "[specify] Worktree already exists at $WORKTREE_DIR, reusing"
        else
...
# Determine where specs live: in the worktree if one was created, otherwise in the current repo
